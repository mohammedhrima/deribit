...
int main() {
    ...
    if(!client_id || !client_secret) {
        std::cerr << "Please set DERIBIT_CLIENT_ID and DERIBIT_CLIENT_SECRET environment variables" << std::endl;
        return 1;
    }
    ...
    // Authenticate
    std::ostringstream auth_message;
    auth_message << R"({
        "jsonrpc": "2.0",
        "id": 1,
        "method": "public/auth",
    ...
    if(!client.send(auth_message.str())) {
